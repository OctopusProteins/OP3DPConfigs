...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  void lcd_main_menu() {
    ...
    else
    {
      ...
      MENU_ITEM(submenu, "Load/Unload Filament", lcd_filament_menu);
      MENU_ITEM(submenu, MSG_PREPARE, lcd_prepare_menu);
      MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);
      if(X_BED_SIZE==1200)
        MENU_ITEM(submenu, MSG_CALIBRATION_120X, lcd_calibration_menu); // 120X Calibration Menu
      else
        MENU_ITEM(submenu, MSG_CALIBRATION, lcd_calibration_menu); // 60 Calibration Menu
    }
  ...
  void lcd_cooldown() {
    ...
    static int16_t ubl_storage_slot = 0,
               ...
               x_plot = 0,
               y_plot = 0;
    static float  nozzle_diameter = 0.4,
                  layer_height = 0.3;
    ...
    void _lcd_ubl_validate_mesh() {
      START_MENU();
      MENU_BACK("Return");
	    MENU_ITEM(gcode, "PLA, Nozzle:0.4", PSTR("G28\nG26 C S0.4 H215 L0.2"));
      MENU_ITEM(gcode, "PLA, Nozzle:0.6", PSTR("G28\nG26 C S0.6 H230 L0.3"));
	    MENU_ITEM(gcode, "PLA, Nozzle:0.8", PSTR("G28\nG26 C S0.8 H230 L0.4"));
      MENU_ITEM(gcode, "PLA, Nozzle:1.0", PSTR("G28\nG26 C S1.0 H230 L0.5"));
      MENU_ITEM(function, MSG_WATCH, lcd_return_to_status);
      END_MENU();
    }
    ...
    void _lcd_ubl_step_by_step() {
      ...
      MENU_ITEM(gcode, "3 " MSG_UBL_ACTIVATE_MESH, PSTR("G29 A\n M300 S440 P200"));
      MENU_ITEM(function, "4 " MSG_UBL_SAVE_MESH, _lcd_ubl_save_mesh_cmd);
      END_MENU();
    }
    ...
    void _lcd_ubl_level_bed() {
      ...
      MENU_ITEM(submenu, MSG_UBL_VALIDATE_MESH_MENU, _lcd_ubl_validate_mesh);
      MENU_ITEM(gcode, "Send info to PC", PSTR("G29 W"));
      MENU_ITEM(gcode, "Send mesh to PC", PSTR("G29 T"));
      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float3, MSG_Z_FADE_HEIGHT, &new_z_fade_height, 0, 100, _lcd_set_z_fade_height);
...
/**
  ...
  void lcd_calibration_menu() {
    START_MENU();
    MENU_BACK(MSG_MAIN);
    MENU_ITEM(submenu, "Probe Calibration", lcd_calibration_inductive_menu);
    MENU_ITEM(submenu, "Tilt Calibration", lcd_calibration_tilt_menu);
    MENU_ITEM(submenu, MSG_UBL_LEVEL_BED, _lcd_ubl_level_bed);
    MENU_ITEM(submenu, "Z Offset Calibration", lcd_calibration_zoffset_menu);
    END_MENU();
  }
  ...
  void lcd_calibration_inductive_menu()
  {
    START_MENU();
    MENU_BACK("Return");
    MENU_ITEM(gcode, "1. Start Process", PSTR("G28 X Y\n G1 X300 Y50 F5000\n M211 S0\n G28 Z"));//M211 software endstops
    MENU_ITEM(submenu,"2. Adjust Height(Z)", lcd_move_get_z_amount);
    MENU_ITEM(gcode, "3. Save Settings", PSTR("M211 S1\n G28"));
    END_MENU();
  }
...
/**
  ...
  void lcd_calibration_tilt_menu() {
    ...
    if(X_BED_SIZE==610)
      ...
      MENU_ITEM(gcode, "1.Start Process", PSTR("G28 X Y\n G1 X600 Y300 F5000"));
    MENU_ITEM(submenu,"2.Adjust height(Z)", lcd_move_get_z_amount);
    MENU_ITEM(submenu, "3.Calibrate Left", lcd_calibration_tilt_lscrew);
    MENU_ITEM(submenu, "4.Calibrate Right", lcd_calibration_tilt_rscrew);
    END_MENU();
  }
  ...
  void lcd_calibration_tilt_lscrew(){
    ...
    MENU_BACK("Return");
    if(X_BED_SIZE==610)
      MENU_ITEM(gcode, "Go to L.Screw", PSTR("G1 X0 Y300 F5000\n M84"));
    else
    {
      MENU_ITEM(gcode, "1.Go to Rear L.Screw", PSTR("G1 X0 Y600 F5000\n M84"));
      MENU_ITEM(gcode, "2.Go to Front L.Screw", PSTR("G1 X0 Y30 F5000\n M84"));
    }
    END_MENU();
  }
  ...
  void lcd_calibration_tilt_rscrew(){
    ...
    if(X_BED_SIZE==610)
    {
      MENU_ITEM(gcode, "1.Go to Rear R.Screw", PSTR("G1 X610 Y600 F5000\n M84"));
      MENU_ITEM(gcode, "2.Go to Front R.Screw", PSTR("G1 X610 Y30 F5000\n M84"));
    }
    else
    {
      MENU_ITEM(gcode, "1.Go to Rear R.Screw", PSTR("G1 X1200 Y600 F5000\n M84"));
      MENU_ITEM(gcode, "2.Go to Front R.Screw", PSTR("G1 X1200 Y30 F5000\n M84"));
    }
    END_MENU();
  }
...
/**
  ...
  void lcd_calibration_zoffset_menu() {
    START_MENU();
    MENU_BACK("Return");
    MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, "1.Set Temerature", &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15, watch_temp_callback_E0);
    MENU_ITEM_EDIT(float52, "2.Set Z Offset", &zprobe_zoffset, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX);
    MENU_ITEM(gcode, "3.Home X & Y", PSTR("G28 X Y"));
    MENU_ITEM(gcode, "4.Verify Height", PSTR("G28 Z\n G1 X0 Y20 F5000\n G1 Z0.2"));
    if (thermalManager.current_temperature[0]<=175)
		   MENU_ITEM(gcode, "5.Heating,Please wait", PSTR("M300"));
    else
		  MENU_ITEM(gcode, "5.Print Test Line", PSTR("G92 E0\n G1 X200 E35 F500\n G92 E0\n G1 E-1 F1000\n G28 X Y"));//E125
    MENU_ITEM(function, "6.Save Z Offset", lcd_store_settings);
    END_MENU();
  }
  /**
   *
...
/**
   *
   * "Load/Unload Filament" submenu
   *
   */
  void lcd_filament_e0_menu();
  void lcd_filament_e1_menu();
  
  void lcd_filament_menu(){
    START_MENU();
    MENU_BACK(MSG_MAIN);
	  MENU_ITEM(gcode, "Go to the Front", PSTR("G28\n G1 X300 Y50 F5000"));
    MENU_ITEM(submenu, "Load/Unload E0", lcd_filament_e0_menu);
    MENU_ITEM(submenu, "Load/Unload E1", lcd_filament_e1_menu);
    END_MENU();
  }
